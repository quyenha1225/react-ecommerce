    ...
    -- Chạy sau schema_updated.sql. Có thể chạy lại nhiều lần mà không tạo dữ liệu trùng.
    -- Bản cuối: tương thích trigger kho, RETURN, promotion, final_unit_price và review xác minh.
    ...
    -- Nhà cung cấp được UPSERT trước khi ghi phiếu nhập kho.
    ...
    -- Không INSERT trực tiếp stock_quantity vào variant_inventory.
    -- Trigger trg_inventory_after_insert trong schema_updated.sql sẽ tự tạo/cộng tồn kho.
    -- Ghi lại đúng các phiếu seed còn thiếu trong bảng tạm để biết variant nào được tạo tồn ở lần chạy này.
    DROP TEMPORARY TABLE IF EXISTS tmp_seed_new_inventory;
    CREATE TEMPORARY TABLE tmp_seed_new_inventory AS
    SELECT
      p.product_id,
      pv.variant_id,
      sp.supplier_id,
      ...
      s.stock_quantity,
      s.reserved_quantity,
      ROUND(p.base_price * s.cost_rate, 2) AS unit_cost,
      s.transaction_note
    FROM tmp_seed_inventory s
    JOIN products p ON p.product_slug = s.product_slug
    ...
    WHERE NOT EXISTS (
      ...
      WHERE it.product_id = p.product_id
        AND it.variant_id = pv.variant_id
        AND it.inventory_type_id = itt.inventory_type_id
        AND it.transaction_note = s.transaction_note
    );

    -- Phiếu kho là dữ liệu lịch sử: chỉ thêm các dòng đã xác định là còn thiếu.
    -- Trigger sẽ tạo/cộng stock_quantity trong cùng transaction.
    ...
    SELECT
      product_id, variant_id, supplier_id, staff_user_id, inventory_type_id,
      stock_quantity, unit_cost, transaction_note
    FROM tmp_seed_new_inventory;

    -- Khởi tạo reserved_quantity = 0. Sau khi tạo đơn mẫu, seed sẽ tính lại
    -- lượng giữ hàng từ các đơn PENDING/CONFIRMED/PACKING.
    UPDATE variant_inventory vi
    JOIN tmp_seed_new_inventory ni ON ni.variant_id = vi.variant_id
    SET vi.reserved_quantity = 0;
    ...
    -- ============================================================================
    -- 8. KHUYẾN MÃI MẪU
    -- Áp dụng một khuyến mãi có lợi nhất; giá giảm thực tế được snapshot vào order_items.
    -- ============================================================================
    INSERT INTO promotions(
      promotion_code, promotion_name, discount_type, discount_value,
      max_discount_amount, start_at, end_at, promotion_status
    ) VALUES
    ('JULY_LAPTOP_10', 'Tháng 7 giảm 10% laptop', 'PERCENT', 10, 3000000,
    '2026-07-01 00:00:00', '2026-07-31 23:59:59', 'ACTIVE'),
    ('JULY_PHONE_5', 'Tháng 7 giảm 5% điện thoại', 'PERCENT', 5, 1500000,
    '2026-07-01 00:00:00', '2026-07-31 23:59:59', 'ACTIVE'),
    ('JULY_ACCESSORY_200K', 'Tháng 7 giảm 200.000 phụ kiện', 'FIXED', 200000, NULL,
    '2026-07-01 00:00:00', '2026-07-31 23:59:59', 'ACTIVE'),
    ('JULY_COMPONENT_5', 'Tháng 7 giảm 5% linh kiện', 'PERCENT', 5, 800000,
    '2026-07-01 00:00:00', '2026-07-31 23:59:59', 'ACTIVE'),
    ('JULY_MONITOR_7', 'Tháng 7 giảm 7% màn hình', 'PERCENT', 7, 1000000,
    '2026-07-01 00:00:00', '2026-07-31 23:59:59', 'ACTIVE'),
    ('ROG_G16_VARIANT_4M', 'Giảm riêng 4 triệu ASUS ROG Strix G16', 'FIXED', 4000000, NULL,
    '2026-07-01 00:00:00', '2026-07-31 23:59:59', 'ACTIVE')
    ON DUPLICATE KEY UPDATE
    ...
    SELECT pr.promotion_id, p.product_id
    FROM promotions pr
    JOIN categories c ON c.category_slug = CASE pr.promotion_code
      WHEN 'JULY_LAPTOP_10' THEN 'laptop'
      WHEN 'JULY_PHONE_5' THEN 'dien-thoai'
      WHEN 'JULY_ACCESSORY_200K' THEN 'phu-kien'
      WHEN 'JULY_COMPONENT_5' THEN 'linh-kien-pc'
      WHEN 'JULY_MONITOR_7' THEN 'man-hinh'
    END
    JOIN products p ON p.category_id = c.category_id
    WHERE pr.promotion_code IN (
      'JULY_LAPTOP_10', 'JULY_PHONE_5', 'JULY_ACCESSORY_200K',
      'JULY_COMPONENT_5', 'JULY_MONITOR_7'
    );

    INSERT IGNORE INTO promotion_variants(promotion_id, product_id, variant_id)
    SELECT pr.promotion_id, p.product_id, pv.variant_id
    FROM promotions pr
    JOIN products p ON p.product_slug = 'asus-rog-strix-g16'
    JOIN product_variants pv ON pv.product_id = p.product_id AND pv.is_default = TRUE
    WHERE pr.promotion_code = 'ROG_G16_VARIANT_4M';
    ...
    -- Tính giá snapshot cho từng đơn mẫu. Nếu có nhiều khuyến mãi, lấy mức giảm lớn nhất.
    DROP TEMPORARY TABLE IF EXISTS tmp_seed_order_pricing;
    CREATE TEMPORARY TABLE tmp_seed_order_pricing AS
    SELECT
      s.order_code,
      p.product_id,
      pv.variant_id,
      s.quantity,
      CAST(p.base_price + pv.additional_price AS DECIMAL(15,2)) AS unit_price,
      CAST(COALESCE(MAX(
        LEAST(
          CASE
            WHEN ap.discount_type = 'PERCENT'
              THEN ROUND((p.base_price + pv.additional_price) * ap.discount_value / 100, 2)
            WHEN ap.discount_type = 'FIXED'
              THEN ap.discount_value
            ELSE 0
          END,
          COALESCE(ap.max_discount_amount, p.base_price + pv.additional_price),
          p.base_price + pv.additional_price
        )
      ), 0) AS DECIMAL(15,2)) AS discount_amount
    FROM tmp_seed_orders s
    JOIN products p ON p.product_slug = s.product_slug
    JOIN product_variants pv ON pv.product_id = p.product_id AND pv.is_default = TRUE
    LEFT JOIN (
      SELECT
        pr.promotion_id, pp.product_id, NULL AS variant_id,
        pr.discount_type, pr.discount_value, pr.max_discount_amount,
        pr.start_at, pr.end_at, pr.promotion_status
      FROM promotions pr
      ...
      SELECT
        pr.promotion_id, pvmap.product_id, pvmap.variant_id,
        pr.discount_type, pr.discount_value, pr.max_discount_amount,
        pr.start_at, pr.end_at, pr.promotion_status
      FROM promotions pr
      JOIN promotion_variants pvmap ON pvmap.promotion_id = pr.promotion_id
    ) ap ON ap.product_id = p.product_id
        AND (ap.variant_id IS NULL OR ap.variant_id = pv.variant_id)
        AND ap.promotion_status = 'ACTIVE'
        AND s.created_at BETWEEN ap.start_at AND ap.end_at
    GROUP BY
      s.order_code, p.product_id, pv.variant_id, s.quantity,
      p.base_price, pv.additional_price;
    ...
    JOIN tmp_seed_order_pricing price ON price.order_code = o.order_code
    SET oi.product_id = price.product_id,
        oi.variant_id = price.variant_id,
        oi.ordered_quantity = price.quantity,
        oi.unit_price_at_order = price.unit_price,
        oi.discount_amount_at_order = price.discount_amount,
        oi.final_unit_price = price.unit_price - price.discount_amount;

    INSERT INTO order_items(
      order_id, product_id, variant_id, ordered_quantity,
      unit_price_at_order, discount_amount_at_order, final_unit_price
    )
    SELECT o.order_id, price.product_id, price.variant_id, price.quantity,
          price.unit_price, price.discount_amount,
          price.unit_price - price.discount_amount
    FROM tmp_seed_order_pricing price
    JOIN orders o ON o.order_code = price.order_code
    WHERE NOT EXISTS (
      SELECT 1 FROM order_items oi WHERE oi.order_id = o.order_id
    );

    -- Đồng bộ kho với đơn mẫu:
    -- SHIPPING/DELIVERED đã xuất kho; PENDING/CONFIRMED/PACKING đang giữ hàng.
    INSERT INTO inventory_transactions(
      product_id, variant_id, supplier_id, staff_user_id, inventory_type_id,
      transaction_quantity, unit_cost, transaction_note, transaction_at
    )
    SELECT
      oi.product_id,
      oi.variant_id,
      ...
      oi.ordered_quantity,
      NULL,
      CONCAT('Xuất kho đơn ', o.order_code),
      o.order_created_at
    FROM tmp_seed_orders s
    JOIN orders o ON o.order_code = s.order_code
    JOIN order_statuses os ON os.order_status_id = o.order_status_id
    JOIN order_items oi ON oi.order_id = o.order_id
    JOIN users staff ON staff.user_email = 'staff.kho@cnpm.local'
    JOIN inventory_transaction_types itt ON itt.inventory_type_code = 'OUT'
    WHERE os.order_status_code IN ('SHIPPING', 'DELIVERED')
      AND NOT EXISTS (
        SELECT 1
        FROM inventory_transactions it
        WHERE it.variant_id = oi.variant_id
          AND it.inventory_type_id = itt.inventory_type_id
          AND it.transaction_note = CONCAT('Xuất kho đơn ', o.order_code)
      );
    ...
    LEFT JOIN (
      SELECT
        oi.variant_id,
        SUM(oi.ordered_quantity) AS reserved_quantity
      FROM tmp_seed_orders s
      JOIN orders o ON o.order_code = s.order_code
      JOIN order_statuses os ON os.order_status_id = o.order_status_id
      JOIN order_items oi ON oi.order_id = o.order_id
      WHERE os.order_status_code IN ('PENDING', 'CONFIRMED', 'PACKING')
      GROUP BY oi.variant_id
    ) reserved ON reserved.variant_id = vi.variant_id
    SET vi.reserved_quantity = LEAST(
      vi.stock_quantity,
    ...
    -- ============================================================================
    -- 10. ĐÁNH GIÁ MẪU ĐÃ XÁC MINH
    -- Chỉ tạo review từ đơn của chính khách hàng, trạng thái DELIVERED và đúng variant đã mua.
    -- Trigger trong schema sẽ tự đặt is_verified_purchase = TRUE.
    -- ============================================================================
    ...
    SET pr.rating = CASE MOD(pr.product_id + pr.user_id, 3) WHEN 0 THEN 4 ELSE 5 END,
        pr.review_title = 'Trải nghiệm sản phẩm',
        pr.review_content = 'Sản phẩm đúng mô tả, đóng gói tốt và sử dụng ổn định.',
        pr.review_status = 'APPROVED',
        pr.helpful_count = MOD(pr.product_id + pr.user_id, 6)
    WHERE o.customer_id = pr.user_id
      AND os.order_status_code = 'DELIVERED'
      AND EXISTS (
        SELECT 1 FROM tmp_seed_orders s WHERE s.order_code = o.order_code
      );

    INSERT INTO product_reviews(
      product_id, user_id, order_id, variant_id, rating,
      review_title, review_content, is_verified_purchase,
      review_status, helpful_count
    )
    SELECT
      oi.product_id,
      o.customer_id,
      o.order_id,
      oi.variant_id,
      CASE MOD(oi.product_id + o.customer_id, 3) WHEN 0 THEN 4 ELSE 5 END,
      'Trải nghiệm sản phẩm',
      'Sản phẩm đúng mô tả, đóng gói tốt và sử dụng ổn định.',
      TRUE,
      'APPROVED',
      MOD(oi.product_id + o.customer_id, 6)
    FROM tmp_seed_orders s
    JOIN orders o ON o.order_code = s.order_code
    JOIN order_statuses os ON os.order_status_id = o.order_status_id
    JOIN order_items oi ON oi.order_id = o.order_id
    WHERE os.order_status_code = 'DELIVERED'
      AND o.customer_id = (
        SELECT u.user_id FROM users u WHERE u.user_email = s.customer_email
      )
      AND NOT EXISTS (
        SELECT 1
        FROM product_reviews pr
        WHERE pr.user_id = o.customer_id
          AND pr.order_id = o.order_id
          AND pr.product_id = oi.product_id
      );

    -- Đồng bộ lại số lượng và điểm trung bình của review đã được duyệt.
    UPDATE products p
    LEFT JOIN (
    ...
    -- ============================================================================
    -- 11. PHÂN BỔ NHÂN VIÊN, CẤU HÌNH AI VÀ AUDIT
    -- ============================================================================
    UPDATE products p
    JOIN (
