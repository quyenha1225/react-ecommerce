...
export class AiService {
  ...
  private readonly answerCache = new Map<
    string,
    { data: AiSearchResultModel; expiresAt: number }
  >();
  constructor(
  ...
  async search(
    ...
    if (cached && cached.expiresAt > Date.now())
      return { ...cached.data, latencyMs: Date.now() - startedAt };
    const catalog = await this.getCatalog();

    if (!useAI) {
      const normalized = this.normalize(query),
        tokens = normalized.split(/\s+/).filter((x) => x.length > 1);
      const products = catalog
        .map((p) => ({ p, score: this.score(p, tokens, normalized) }))
        .filter((x) => x.score > 0)
        .sort((a, b) => b.score - a.score)
        .slice(0, 10)
        .map((x) => this.publicProduct(x.p));
      const result: AiSearchResultModel = {
        mode: 'standard',
        query,
        products,
      ...
      this.setAnswerCache(cacheKey, result, 60_000);
      return result;
    }
      ...
      try {
        result = await this.withDeadline(
          this.geminiRecommendation(
            query,
            this.prefilterCatalog(query, catalog),
            apiKey,
          ),
          5_000,
        );
        ...
        result = this.heuristicRecommendation(query, catalog);
      }
    result.latencyMs = Date.now() - startedAt;
    this.setAnswerCache(
      cacheKey,
      result,
      result.aiAvailable ? 180_000 : 30_000,
    );
    return result;
  }
  ...
  private async getCatalog(): Promise<ProductRow[]> {
    ...
    this.catalogCache = { data, expiresAt: Date.now() + 30_000 };
    return data;
  }
  ...
  private async geminiRecommendation(
    ...
    const safeCatalog = catalog.map((p) => ({
      id: Number(p.id),
      name: p.name,
      category: p.category,
      brand: p.brand,
        ...
        body: JSON.stringify({
          ...
          generationConfig: {
            ...
            responseSchema: {
              ...
              required: ['summary', 'productIds', 'criteria'],
            },
          },
        }),
      },
    ...
    if (!response.ok) {
      ...
      throw new ServiceUnavailableException(
        'Gemini đang bận, vui lòng thử lại',
      );
    }
    ...
    const ids: number[] = [
      ...new Set<number>(
        (Array.isArray(parsed.productIds) ? parsed.productIds : []).map(
          (id: any) => Number(id),
        ),
      ),
    ].slice(0, 5);
    ...
    const products = ids
      .map((id) => byId.get(id))
      .filter(Boolean)
      .map((p) => this.publicProduct(p!));
    return {
      mode: 'ai',
      provider: 'gemini',
      aiAvailable: true,
      query,
      summary: this.compactText(parsed.summary, 220),
      criteria: Array.isArray(parsed.criteria)
        ? parsed.criteria.slice(0, 4).map((x: any) => this.compactText(x, 60))
        : [],
      products,
      latencyMs: 0,
    };
  ...
  private heuristicRecommendation(
    ...
    const normalized = this.normalize(query),
      tokens = normalized.split(/\s+/).filter((x) => x.length > 1);
    const millions = normalized.match(/(\d{1,3})\s*(?:trieu|tr)/);
    const budget = millions ? Number(millions[1]) * 1_000_000 : null;
    const minimumBudget = /(?:tren|tu)\s*\d/.test(normalized);
    const ranked = catalog
      .map((p) => {
        let score = this.score(p, tokens, normalized);
        const category = this.normalize(p.category);
        if (normalized.includes(category)) score += 20;
        if (
          budget &&
          (minimumBudget
            ? Number(p.price) >= budget
            : Number(p.price) <= budget)
        )
          score += 10;
        if (
          budget &&
          (minimumBudget ? Number(p.price) < budget : Number(p.price) > budget)
        )
          score -= 8;
        if (Number(p.stock) > 0) score += 2;
        return { p, score };
      })
      .sort((a, b) => b.score - a.score)
      .slice(0, 5);
    return {
      ...
      aiAvailable: false,
      query,
      summary: 'Gợi ý nhanh theo nhu cầu, ngân sách và tồn kho hiện tại.',
      criteria: [
        budget
          ? `${minimumBudget ? 'Giá từ' : 'Ngân sách đến'} ${budget.toLocaleString('vi-VN')} ₫`
          : 'Phù hợp từ khóa',
        'Ưu tiên còn hàng',
      ],
      products: ranked.map((x) => this.publicProduct(x.p)),
      latencyMs: 0,
    };
  }

  private prefilterCatalog(query: string, catalog: ProductRow[]) {
    const normalized = this.normalize(query),
      tokens = normalized.split(/\s+/).filter((x) => x.length > 1),
      ranked = catalog
        .map((p) => ({
          p,
          score:
            this.score(p, tokens, normalized) + (Number(p.stock) > 0 ? 1 : 0),
        }))
        .sort((a, b) => b.score - a.score);
    const relevant = ranked.filter((x) => x.score > 1);
    return (relevant.length >= 8 ? relevant : ranked)
      .slice(0, 24)
      .map((x) => x.p);
  }
  private compactText(value: any, max: number) {
    const text = String(value || '')
      .replace(/\s+/g, ' ')
      .trim();
    return text.length > max ? `${text.slice(0, max - 1).trim()}…` : text;
  }
  private setAnswerCache(key: string, data: AiSearchResultModel, ttl: number) {
    if (this.answerCache.size > 100) this.answerCache.clear();
    this.answerCache.set(key, { data, expiresAt: Date.now() + ttl });
  }
  ...
  private score(p: ProductRow, tokens: string[], full: string) {
    const hay = this.normalize(
      `${p.name} ${p.category} ${p.brand || ''} ${p.description || ''}`,
    );
    let score = hay.includes(full) ? 20 : 0;
  ...
  private checkRateLimit(key: string, limit: number) {
    ...
    if (current && current.resetAt > now && current.count >= limit)
      throw new HttpException(
        'Bạn tìm kiếm quá nhanh, vui lòng đợi một phút',
