...
const resources: Record<string, Config> = {
  ...
  stock:{table:'vw_product_stock',id:'product_id',scope:'STAFF',permission:'MANAGE_INVENTORY',select:['product_id','product_name','variant_id','variant_name','stock_quantity','reserved_quantity','available_quantity'],writable:[],order:'product_id',readonly:true},
...
export class ManagementService {
  ...
  async productDetail(id:number,user:AuthUser){ this.config('products',user); const product=await this.one('products',id,user); const [images,variants,attributes,suppliers,stock,sales]=await Promise.all([this.db.query(`SELECT * FROM product_images WHERE product_id=? ORDER BY sort_order`,[id]),this.db.query(`SELECT pv.*,vi.stock_quantity,vi.reserved_quantity FROM product_variants pv LEFT JOIN variant_inventory vi ON vi.variant_id=pv.variant_id WHERE pv.product_id=?`,[id]),this.db.query(`SELECT pa.*,pav.attribute_value FROM product_attribute_values pav JOIN product_attributes pa ON pa.attribute_id=pav.attribute_id WHERE pav.product_id=? ORDER BY pa.display_order`,[id]),this.db.query(`SELECT ps.*,s.supplier_name FROM product_suppliers ps JOIN suppliers s ON s.supplier_id=ps.supplier_id WHERE ps.product_id=?`,[id]),this.db.query(`SELECT COALESCE(SUM(available_quantity),0) current_stock FROM vw_product_stock WHERE product_id=?`,[id]),this.db.query(`SELECT total_sold FROM vw_best_selling_products WHERE product_id=?`,[id])]); return {...product,images,variants,attributes,suppliers,current_stock:Number(stock[0]?.current_stock||0),total_sold:Number(sales[0]?.total_sold||0)}; }
  async dashboard(user:AuthUser){
    ...
    const [customers,revenue,orders,profit,newCustomers,topProducts,lowStock,recentOrders,inventory,importValue,salesCost,dailySales]=await Promise.all([
      ...
      this.db.query(`SELECT product_id,MAX(product_name) product_name,SUM(available_quantity) current_stock FROM vw_product_stock GROUP BY product_id HAVING current_stock<=5 ORDER BY current_stock ASC LIMIT 8`),
